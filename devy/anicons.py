...
def splice_ep_name_predicions_with_data(path):
    ...
    api_episodes = lock_file_data['episode_names']
    local_episodes= lock_file_data['local_ep_data']
    
    print(f'{c.b_black}{api_episodes}\n\n\n{local_episodes}{c.o}')
    
    print(f'Internet data :\n')
    for item in api_episodes:
        print(f'{c.b_blue}{api_episodes[item]}{c.o}\n')
    print(f'Local data :\n')
    for item in local_episodes:
        print(f'{c.b_purple}{item}{c.o}\n')


    episodes = {}

    def check_delta(lock_file_data):
        data = lock_file_data['local_ep_data']
        print(f'\n{c.red}--- PREDICTION CORRELATION  ---{c.o}\n')
        delta = []
        for ep in data :
            prediction_data = []
    ...
    lock_file_data['episodes'] = episodes
        ...
        if node.is_dir():
            ...
            if Path.joinpath(node,'Ξ.lock').exists() :
                ...
                if data['is_verified'] == False:
                    ...
                    print(f'{c.green}[Lock file] Anime is already asserted. {c.o}\n')
                if data['api_fetched'] == False:
                    generate_api_data_from_lock(node)
                else:
                    print(f'{c.green}[Lock file] Api data is already fetched. {c.o}\n')
